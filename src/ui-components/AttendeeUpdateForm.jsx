...
export default function AttendeeUpdateForm(props) {
  ...
  React.useEffect(() => {
    const queryData = async () => {
      ...
      const linkedEventAttendees =
        (record && (await record.EventAttendees?.toArray())) || [];
  ...
  const getDisplayValue = {
    EventAttendees: (r) => `${r?.email ? r?.email + " - " : ""}${r?.id}`,
  };
  const validations = {
    EventAttendees: [],
  };
    ...
    >
      <ArrayField
        ...
        runValidationTasks={async () =>
          await runValidationTasks("EventAttendees", currentEventAttendeesValue)
        }
